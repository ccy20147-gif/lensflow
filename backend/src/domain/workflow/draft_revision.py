...
_PRESENTATION_NODE_KEYS = frozenset({
    "position", "selected", "dragging", "dimensions", "width", "height",
    "computedPosition", "resizing", "hidden", "zIndex",
})
...
def normalize_graph_and_layout(
    ...
    """Split Vue Flow presentation state from executable graph content.

    Older clients put ``position`` directly on graph nodes.  Accept that
    payload for compatibility, but persist it exclusively under ``layout``;
    execution hashing must never observe a drag, selection, or dimensions
    update.  Edge handles deliberately remain semantic because changing the
    bound port changes the data contract.
    """
    normalized_layout = dict(layout or {})
    positions = dict(normalized_layout.get("nodes") or normalized_layout.get("positions") or {})
    raw_nodes = (graph or {}).get("nodes", [])
    # The persisted Vue Flow contract is a list.  Preserve the legacy mapping
    # shape used by older API clients/tests while applying the same stripping.
    items = raw_nodes.items() if isinstance(raw_nodes, dict) else enumerate(raw_nodes)
    normalized_nodes: list[Any] | dict[str, Any] = {} if isinstance(raw_nodes, dict) else []
    for raw_key, raw_node in items:
        if not isinstance(raw_node, dict):
            if isinstance(normalized_nodes, dict):
                normalized_nodes[str(raw_key)] = raw_node
            else:
                normalized_nodes.append(raw_node)
            continue
        node = {key: value for key, value in raw_node.items() if key not in _PRESENTATION_NODE_KEYS}
        node_id = str(node.get("id", ""))
        position = raw_node.get("position")
        if node_id and isinstance(position, dict):
            positions[node_id] = position
        if isinstance(normalized_nodes, dict):
            normalized_nodes[str(raw_key)] = node
        else:
            normalized_nodes.append(node)
    normalized_graph = {
        **{key: value for key, value in (graph or {}).items() if key not in {"nodes", "layout", "viewport"}},
        "nodes": normalized_nodes,
        "edges": list((graph or {}).get("edges", [])),
    }
    if positions:
        normalized_layout["nodes"] = positions
        normalized_layout.pop("positions", None)
    return normalized_graph, normalized_layout


def _semantic_graph(graph: dict[str, Any]) -> dict[str, Any]:
    """Return graph data eligible for graph/execution hashes."""
    normalized, _ = normalize_graph_and_layout(graph, {})
    return normalized
...
def compute_graph_hash(
    ...
    hasher.update(json.dumps(_semantic_graph(graph), sort_keys=True, default=str).encode())
    hasher.update(b"|")
