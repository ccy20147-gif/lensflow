...
class RegistryService:
    ...
    def get_definition(
        ...
        or latest draft if no active exists. Does NOT return retired
        definitions by default (use revision_id to access retired)."""
        entry = self._definitions.get(node_type_id)
        if not entry:
            return None
        ...
        # Prefer active, then latest draft (never fall back to retired)
        active = entry.get("active")
    ...
    def generate_snapshot(self) -> RegistrySnapshot:
        ...
        for ndr in sorted(node_defs.keys()):
            raw.append(node_defs[ndr].model_dump_json())
    ...
    def list_snapshots(self) -> list[RegistrySnapshot]:
        ...
        return sorted(self._snapshots.values(), key=lambda s: str(s.created_at or ''), reverse=True)
