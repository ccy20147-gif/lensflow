...
- 2026-07-16 Foundation 验收：批次 C 的持久 Run/NodeRun/Attempt、epoch fencing、dispatch/result outbox、unknown 对账、取消隔离、输出绑定 owner_scope 与固定持久 plan_json 边界均有 PostgreSQL 回归覆盖；`8c9d0e1f2a3b` 增加 dispatch/result outbox dedupe_key 及部分唯一索引。独立执行迁移、运行时/回调/局部重跑回归（45 passed）、ruff 与 mypy 通过。V0 最小持久运行和 V1 lease/复杂恢复仍在交付中，状态保持 `in_delivery`。
- 2026-07-16 V0 hardening 验收：`9b0c1d2e3f4a` 为 rejected callback audit 增加部分唯一 dedupe 约束；phase-2 fence 重验、lease heartbeat/过期后 replacement attempt、partial-run 闭包校验与 discarded 审计已由 PostgreSQL 专项（22 passed）独立验证。此为 V0 加固切片，不关闭 V0 全范围或 V1 复杂恢复。
